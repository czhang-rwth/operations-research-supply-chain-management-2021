...
def solve_model(T,P,c,h_pp,h_up,d):
    ...
    model.setObjective(quicksum(quicksum(h_up[p] * ((i_up[p,t-1] + i_up[p,t]) / 2) + h_pp[p] * ((i_pp[p,t-1] + i_pp[p,t]) / 2) for t in range(1, T + 1)) for p in P))
...
production, packaging, inventory_unpacked, inventory_packed = solve_model(T,P,c,h_pp,h_up,d)
